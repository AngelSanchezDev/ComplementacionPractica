...
def test_seed_generar(tmp_path, monkeypatch):
    ...
    antes = db.conectar(tmp_path / "seed3.db")
    total_antes = db.contar_clientes(antes)
    antes.close()

    assert seed.main(["generar", "50"]) == 0

    conn = db.conectar(tmp_path / "seed3.db")
    assert db.contar_clientes(conn) == total_antes + 50
    conn.close()
